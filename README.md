...
- **アカウント自己削除**（Phase 5.5）: `/settings` から通常アカウントを完全削除可能。削除前に「自分が唯一のオーナーのサークル」が残っていないか自動チェックし、残っていれば block。所属サークルから自動脱退してから Firebase Auth とユーザードキュメントを削除する。過去のトーナメント参加記録とシーズン戦績は履歴として残る。匿名（ゲスト）はログアウト時に従来通り自動削除されるため対象外
