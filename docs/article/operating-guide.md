...
- **おすすめ条件**: 1200×630 比率 / jpeg / 5MB 以下。**単色領域があると文字 box が見やすい**。明暗が極端にコントラストする画像は box overlay でも読みづらい場合があるので、シンプルな写真やロゴ寄りの画像が無難
- **ライト / ダーク テーマの使い分け**: 明るい / 中間明度の背景画像 → 「ライト」（黒テキスト + 白半透明 box）/ 暗い背景画像 → 「ダーク」（白テキスト + 紺半透明 box）
- **プレビュー**: 設定画面のプレビュー領域に実画像 + scrim + box overlay がそのまま映ります。保存前にここで「文字が読めるか」を確認
