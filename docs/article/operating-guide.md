...
### 結果カードの背景画像を差し替える（オーナーのみ）

サークル詳細 → 設定タブ → 「優勝者カード背景画像」 / 「シーズン戦績カード背景画像」 から、結果カード PNG の背景画像を差し替えられます（共有リンクで誰でも閲覧可能になります）。

- **おすすめ条件**: 1200×630 比率 / jpeg / 5MB 以下。中央付近に大きな優勝者名、最下部に情報ボックス（サークル名 / 開催日 / 参加人数 / アプリ名）が乗るので、それらが乗るあたりに**主役の被写体や文字が来ない画像**が無難
- **ライト / ダーク テーマの使い分け**: 明るい / 中間明度の背景画像 → 「ライト」（黒テキスト + 白い縁取り + 最下部は白半透明ボックス）/ 暗い背景画像 → 「ダーク」（白テキスト + 黒い縁取り + 最下部は紺半透明ボックス）
- **プレビュー**: 設定画面のプレビュー領域に実画像 + 上下グラデ scrim + テキスト縁取り + 最下部の情報ボックスがそのまま映ります。保存前にここで「文字が読めるか」「主役の被写体が隠れていないか」を確認
- **解除**: 「背景を解除」ボタン → 確認ダイアログ → 「背景を解除する」。固定グラデーション背景に戻ります
