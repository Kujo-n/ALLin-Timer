...
- ドメインコード（prefix）を必ず付与:
  ...
  - `group/*` — group 操作起因（Phase 1 (08-auto-group-join-on-entry) でトーナメント受付経由の自動所属に `group/auto-join-failed`（self-add が rule で deny され、再 probe でもメンバーになっていない恒久的失敗）を追加）
